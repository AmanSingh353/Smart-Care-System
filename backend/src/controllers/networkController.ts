  ...
  async list(req: Request, res: Response) {
    try {
      const hospitals = await listConnectedHospitals({
        ...
        includeLocal: String(req.query.includeLocal || "") === "true",
      });
...
export const assistanceController = {
  ...
  async updateStatus(req: Request, res: Response) {
    try {
      ...
      const request = await changeAssistanceStatus(
        String(req.params.id || ""),
        status,
        { role: req.user.role, staffId: req.staffUser.staffId },
        { adminNetwork: req.user.role === "admin" }
      );
