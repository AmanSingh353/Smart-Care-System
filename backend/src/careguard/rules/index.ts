import type { CareGuardPatientSnapshot, CareGuardSignal, CareGuardSignalType, CareGuardSeverity, CareGuardRole } from "../types";
...
function parseRequestedAt(test: CareGuardPatientSnapshot["tests"][0], now: Date): Date {
  ...
  if (fromHm) {
    // If clock time is in the future relative to now, treat as yesterday (overnight demo)
...
/** RULE 1 — Lab result awaiting doctor review */
...
/** RULE 2 — Explicitly critical lab result (never inferred from values) */
...
/** RULE 3 — Delayed lab order (configurable threshold) */
...
/** RULE 4 — Prescription awaiting pharmacy dispensing */
...
/** RULE 5 — Overdue treatment / medication task */
  ...
  evaluate(patient, now) {
    ...
    for (const med of patient.medicines) {
      for (const slot of med.schedule) {
        ...
        const entityId = `${med.id}:${slot.time}`;
        out.push({
...
/** RULE 6 — Discharge blocked by incomplete billing */
...
/** RULE 7 — Allergy / prescription safety review (human review only) */
...
/** Deduplicate key for active signals */
