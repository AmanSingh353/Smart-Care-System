...
import type { CareGuardSignal, CareGuardWorkflowEvent } from "./types";
import {
  defaultCareGuardRules,
  signalKey,
  candidateToPartial,
  type CareGuardRule,
  type RuleCandidate,
...
export class DeterministicRuleProvider implements CareGuardInsightProvider {
  ...
  evaluate(patient: Patient, now: Date = new Date()) {
    return this.rules.flatMap(rule => rule.evaluate(patient, now));
  }
}

/** Future Python AI/analytics provider — not implemented in Phase 4. */
export class PythonAIProvider implements CareGuardInsightProvider {
  readonly name = "PythonAIProvider";
  evaluate(_patient: Patient, _now?: Date): RuleCandidate[] {
    return [];
  }
}

type SignalListener = (event: "created" | "updated", signal: CareGuardSignal) => void;
...
export class CareGuardEngine {
  ...
  getForRole(role: string) {
    ...
    if (r === "ADMIN") {
      return this.getAll().filter(s => s.status === "OPEN" || s.status === "ACKNOWLEDGED");
    }
    return this.getAll().filter(
      s => s.responsibleRole === r && (s.status === "OPEN" || s.status === "ACKNOWLEDGED")
    );
  }

  handleEvent(_event: CareGuardWorkflowEvent, patient: Patient, now: Date = new Date()) {
    return this.evaluatePatient(patient, now);
  }

  evaluatePatient(patient: Patient, now: Date = new Date()) {
    ...
    for (const s of [...this.signals.values()]) {
      ...
      const resolved: CareGuardSignal = { ...s, status: "RESOLVED", updatedAt: nowIso };
  ...
  evaluateMany(patients: Patient[], now: Date = new Date()) {
    return patients.flatMap(p => this.evaluatePatient(p, now));
  }

  acknowledge(signalId: string) {
    return this.setStatus(signalId, "ACKNOWLEDGED");
  }

  resolve(signalId: string) {
    return this.setStatus(signalId, "RESOLVED");
  }

  dismiss(signalId: string) {
    return this.setStatus(signalId, "DISMISSED");
  }

  private setStatus(signalId: string, status: CareGuardSignal["status"]) {
    const s = this.signals.get(signalId);
    if (!s) return undefined;
    const updated: CareGuardSignal = { ...s, status, updatedAt: new Date().toISOString() };
  ...
  serialize(): CareGuardSignal[] {
    return this.getAll();
  }

  hydrate(signals: CareGuardSignal[]) {
    this.signals.clear();
